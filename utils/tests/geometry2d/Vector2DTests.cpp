...
TEST_CASE("[Vector2D] create() method", "[utils][Vector2D]")
{
    ...
    {
        const Vector2D<> vector2D{Vector2D<>::create(2, 4)};
    ...
    {
        const Vector2D<> vector2D{Vector2D<>::create(-2, 3)};
        CHECK(vector2D.getX() == -2);
...
TEST_CASE("[Vector2D] Constructor", "[utils][Vector2D]")
{
    ...
    SECTION("Group constructor")
    {
        SECTION("Positive values")
        {
            const Vector2D<> vector2D{Coord2D<>{2, 4}};
            CHECK(vector2D.getX() == 2);
            CHECK(vector2D.getY() == 4);
            const auto coords{vector2D.getCoordinates()};
            CHECK(coords[0] == 2);
        ...
        SECTION("Positive and negatives values")
        {
            const Vector2D<> vector2D{Coord2D<>{-2, 3}};
            CHECK(vector2D.getX() == -2);
            CHECK(vector2D.getY() == 3);
            const auto coords{vector2D.getCoordinates()};
            CHECK(coords[0] == -2);
